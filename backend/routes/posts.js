	...
	try {
		...
		const friendsPosts = await Promise.all(
			user.followings.map((following) => {
				return Post.find({ user: following.user }).populate(
					"user",
					"name email profilePic followers followings"
				);
			})
...
router.delete("/comments/:postId/:commentId", protect, async (req, res) => {
	try {
		...
		console.error(err.message);
		if (err.kind == "ObjectId") {
			return res.status(400).json({ msg: "Error with comment deletion" });
		}
		return res.status(500).send("Server Error");
	}
});
...
router.put(
	...
	async (req, res) => {
		try {
			...
			console.error(err.message);
			if (err.kind == "ObjectId") {
				return res.status(400).json({ msg: "Error with reply like/unlike" });
			}
			return res.status(500).send("Server Error");
		}
	}
...
router.delete(
	...
	async (req, res) => {
		try {
			...
			console.error(err.message);
			if (err.kind == "ObjectId") {
				return res.status(400).json({ msg: "Error with reply deletion" });
			}
			return res.status(500).send("Server Error");
		}
	}
